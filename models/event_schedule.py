...
class EventSchedule(Base):
    ...
    subscriptions = relationship("Subscription", back_populates="schedule")
    ...
    places_sub = column_property(
        select(func.sum(Subscription.people_number)).where(Subscription.schedule_id == id).scalar_subquery()
    )
